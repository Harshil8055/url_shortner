...
- **Correctness does not rely on this number being large** — it relies on the
  ...
  **hard cap of 5 retries** as a defensive measure against a pathological/misconfigured
  ...
  `ServiceResult.Status.GENERATION_FAILED`, which the controller maps to `500 Internal
...
| `INVALID_URL` | 400 | Malformed/invalid URL on shorten |
| `INVALID_ALIAS` | 400 | Invalid alias characters or length |
| `ALIAS_CONFLICT` | 409 | Alias already taken |
| `NOT_FOUND` | 404 | Unknown code on redirect |
| `GENERATION_FAILED` | 500 | Defensive: code generator exhausted its retry cap (see §4.1) |
...
| Custom alias support, 409 on conflict | `UrlShortenerService` §2.2, `ServiceResult.Status.ALIAS_CONFLICT` |
...
| 404 for unknown codes | `UrlController` + `ServiceResult.Status.NOT_FOUND` |
